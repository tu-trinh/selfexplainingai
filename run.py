...
from package.infrastructure.env_constants import OBJECT_TO_IDX, COLOR_TO_IDX
...
if __name__ == "__main__":
    ...
    env = FullyObsWrapper(p.world_model)
    pol = p._find_optimal_policy()
    obs, _ = env.reset()
    carrying = None
    for act in pol:
        print(get_obs_desc(obs, detail = 4, carrying = carrying))
        if act == 3:
            carrying = env.grid.get(env.front_pos[0], env.front_pos[1])
        elif act == 4:
            carrying = None
        print("CARRYING IS NOW", carrying)

...
class BaseLayout(ABC):
    ...
    def _build_walls_and_doors_based_on_target(self) -> Tuple[int, int, int, int]:
        ...
        # Establish doors
        self.doors = [(Door(is_locked = self.layout in [Layout.UNLOCK_DOOR, Layout.HIDDEN_KEY], color = random.choice(self.allowed_object_colors)), random.choice(wall_positions))]
...
class DistractorsLayout(BaseLayout):
    def initialize_layout(self):
...
class OpenDoorLayout(BaseLayout):
    def initialize_layout(self):
        # Set target objects
        if self.is_single_target:
            self._set_target_start_position()
...
class BlockedDoorLayout(BaseLayout):
    def initialize_layout(self):
        # Set target objects
        if self.is_single_target:
            self._set_target_start_position()
...
class UnlockDoorLayout(BaseLayout):
    def initialize_layout(self):
        # Set target objects
        if self.is_single_target:
            self._set_target_start_position()
...
class HiddenKeyLayout(BaseLayout):
    def initialize_layout(self):
        # Set target objects
        if self.is_single_target:
            self._set_target_start_position()
...
class GoAroundLayout(BaseLayout):
    def initialize_layout(self):
        # Set target objects
        if self.is_single_target:
            self._set_target_start_position()
...
class MultipleRoomsLayout(BaseLayout):
    def initialize_layout(self):

...
from package.infrastructure.env_constants import COLOR_NAMES, MIN_ROOM_SIZE, MAX_ROOM_SIZE, SKILL_PHRASES
...
train_seeds = [1, 2, 3, 5, 7, 9]
val_seeds = [1, 2, 11, 12, 13, 14]
test_seeds = [1, 2, 4, 6, 8, 10]
...
def create_datasets(mismatch):  # loose upper bound: 5 tasks x 12 levels x 2 train/test x 12 sets x 2 secs = 48 min
    ...
    for task in [Task.GOTO, Task.PICKUP]:
        for level in [Level.ROOM_DOOR_KEY, Level.TREASURE_ISLAND]:
            ...
            # Make datasets
            train_dataset = add_to_dataset(task, level, train_colors, train_steps, train_phrases, train_seeds, "train")
            if not testing:
...
def add_to_dataset(task: Task, level: Level, colors: List[str], steps: List[int], phrases: Dict, seeds: List[int], ds: str, ref_colors: List[str] = None, ref_steps: List[int] = None, ref_phrases: List[str] = None) -> Dict:
    ...
    for i, seed in enumerate(seeds):
        if greenlight(task, level, seed):
            ...
            a._build_task_tree()
            a_skills = a.task_tree.get_skill_subset(seed / 10 if seed <= 10 else seed / 20)
            if ds == "train" or i not in [2, 3]:
